# 💻 代码详解

本文档详细解释项目中每个文件的作用和核心代码实现，帮助初学者理解和修改代码。

## 📚 目录

1. [项目文件概览](#项目文件概览)
2. [model.py - U-Net 模型](#modelpy---u-net-模型)
3. [dataset.py - 数据加载](#datasetpy---数据加载)
4. [model_train.py - 模型训练](#model_trainpy---模型训练)
5. [app.py - Web 应用](#apppy---web-应用)
6. [如何修改和扩展](#如何修改和扩展)

---

## 项目文件概览

### 核心代码文件

| 文件 | 行数 | 作用 | 难度 |
|------|-----|------|------|
| **model.py** | ~76 行 | 定义 U-Net 神经网络结构 | ⭐⭐⭐ |
| **dataset.py** | ~152 行 | 加载和预处理数据 | ⭐⭐ |
| **model_train.py** | ~302 行 | 训练模型的主程序 | ⭐⭐⭐⭐ |
| **model_test.py** | ~160 行 | 测试和评估模型 | ⭐⭐ |
| **app.py** | ~221 行 | Web 应用程序 | ⭐⭐⭐ |

### 辅助文件

| 文件 | 作用 |
|------|------|
| **requirements.txt** | Python 依赖包列表 |
| **run_linux.sh** | Linux 启动脚本 |
| **run_windows.bat** | Windows 启动脚本 |
| **create_val_set.py** | 数据集划分脚本 |
| **mean_std.py** | 计算数据集均值和标准差 |

---

## model.py - U-Net 模型

### 文件作用

定义 U-Net 神经网络的结构。U-Net 是一种用于图像分割的深度学习模型。

### 核心组件

#### 1. DoubleConv 模块

**作用**：U-Net 的基本构建块，进行两次卷积操作

```python
class DoubleConv(nn.Module):
    """(卷积 => BN => ReLU) * 2"""
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.double_conv = nn.Sequential(
            # 第一次卷积
            nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1),
            nn.BatchNorm2d(out_channels),  # 批归一化
            nn.ReLU(inplace=True),          # 激活函数

            # 第二次卷积
            nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True)
        )

    def forward(self, x):
        return self.double_conv(x)
```

**参数解释**：
- `in_channels`: 输入通道数（如 RGB 图片 = 3）
- `out_channels`: 输出通道数（特征图数量）
- `kernel_size=3`: 使用 3×3 的卷积核
- `padding=1`: 填充，保持图片尺寸不变

**通俗理解**：
想象你有一张图片，DoubleConv 像是给图片加两层"滤镜"，每层滤镜提取不同的特征（边缘、纹理等）。

#### 2. UNet 主体结构

**整体架构**：U 型结构（先压缩再扩张）

```python
class UNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=2):
        super(UNet, self).__init__()

        # ===== 编码器（下采样路径）=====
        # 逐步提取特征，同时缩小图片尺寸

        self.inc = DoubleConv(n_channels, 64)    # 3 → 64 通道
        self.down1 = nn.Sequential(
            nn.MaxPool2d(2),                      # 尺寸减半
            DoubleConv(64, 128)                   # 64 → 128 通道
        )
        self.down2 = nn.Sequential(
            nn.MaxPool2d(2),                      # 再减半
            DoubleConv(128, 256)                  # 128 → 256 通道
        )
        self.down3 = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleConv(256, 512)
        )
        self.down4 = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleConv(512, 1024)                 # 最深层：1024 通道
        )

        # ===== 解码器（上采样路径）=====
        # 逐步恢复图片尺寸，同时融合高低层特征

        self.up1 = nn.ConvTranspose2d(1024, 512, kernel_size=2, stride=2)
        self.conv1 = DoubleConv(1024, 512)  # 1024 = 512(上采样) + 512(跳跃连接)

        self.up2 = nn.ConvTranspose2d(512, 256, kernel_size=2, stride=2)
        self.conv2 = DoubleConv(512, 256)

        self.up3 = nn.ConvTranspose2d(256, 128, kernel_size=2, stride=2)
        self.conv3 = DoubleConv(256, 128)

        self.up4 = nn.ConvTranspose2d(128, 64, kernel_size=2, stride=2)
        self.conv4 = DoubleConv(128, 64)

        # ===== 输出层 =====
        self.outc = nn.Conv2d(64, n_classes, kernel_size=1)  # 64 → 2 类别
```

**尺寸变化示例**（输入 224×224×3）：

```
输入: [224×224×3]
  ↓ inc
[224×224×64]
  ↓ down1 (MaxPool2d)
[112×112×128]
  ↓ down2
[56×56×256]
  ↓ down3
[28×28×512]
  ↓ down4
[14×14×1024]  ← 最深层（瓶颈）
  ↓ up1
[28×28×512]   ← 上采样 + 跳跃连接
  ↓ up2
[56×56×256]
  ↓ up3
[112×112×128]
  ↓ up4
[224×224×64]
  ↓ outc
输出: [224×224×2]  (2个类别：背景、裂缝)
```

#### 3. 前向传播

**跳跃连接**：编码器的特征直接传递给解码器

```python
def forward(self, x):
    # 编码路径 - 保存中间特征
    x1 = self.inc(x)         # [224×224×64]
    x2 = self.down1(x1)      # [112×112×128]
    x3 = self.down2(x2)      # [56×56×256]
    x4 = self.down3(x3)      # [28×28×512]
    x5 = self.down4(x4)      # [14×14×1024]

    # 解码路径 - 融合特征
    x = self.up1(x5)                    # 上采样: [28×28×512]
    x = torch.cat([x, x4], dim=1)       # 拼接: [28×28×1024]
    x = self.conv1(x)                   # 卷积: [28×28×512]

    x = self.up2(x)                     # [56×56×256]
    x = torch.cat([x, x3], dim=1)       # [56×56×512]
    x = self.conv2(x)                   # [56×56×256]

    x = self.up3(x)                     # [112×112×128]
    x = torch.cat([x, x2], dim=1)       # [112×112×256]
    x = self.conv3(x)                   # [112×112×128]

    x = self.up4(x)                     # [224×224×64]
    x = torch.cat([x, x1], dim=1)       # [224×224×128]
    x = self.conv4(x)                   # [224×224×64]

    # 输出分割结果
    logits = self.outc(x)               # [224×224×2]
    return logits
```

**为什么需要跳跃连接？**
- 编码器：提取高层语义特征（"这是裂缝"）
- 跳跃连接：保留低层细节特征（"裂缝的精确位置"）
- 结合两者：准确定位裂缝边界

---

## dataset.py - 数据加载

### 文件作用

负责读取图片和标注，转换为 PyTorch 可以处理的张量格式。

### 核心类：CrackDataset

```python
class CrackDataset(Dataset):
    """道路裂缝分割数据集"""

    def __init__(self, image_dir, mask_dir, transform=None, img_size=(224, 224)):
        """
        初始化数据集

        Args:
            image_dir: 图片文件夹路径
            mask_dir: 掩膜文件夹路径
            transform: 数据增强变换
            img_size: 图片尺寸 (height, width)
        """
        self.image_dir = image_dir
        self.mask_dir = mask_dir
        self.transform = transform
        self.img_size = img_size

        # 获取所有图片文件名
        self.images = sorted([
            f for f in os.listdir(image_dir)
            if f.endswith(('.jpg', '.png', '.jpeg', '.JPG', '.PNG'))
        ])

        if len(self.images) == 0:
            raise ValueError(f"❌ 在 {image_dir} 中没有找到图片文件！")

        print(f"✓ 加载数据集: {len(self.images)} 张图片")
```

**通俗理解**：
`CrackDataset` 像一个"图书管理员"，它知道所有图片和标注的位置，当你需要某张图片时，它会帮你找到并准备好。

### 数据读取

```python
def __getitem__(self, idx):
    """获取第 idx 个样本"""

    # 1. 读取图片
    img_name = self.images[idx]
    img_path = os.path.join(self.image_dir, img_name)

    # 2. 读取对应的标注（尝试多种扩展名）
    mask_name = os.path.splitext(img_name)[0]  # 去掉扩展名
    mask_path = None
    for ext in ['.png', '.jpg', '.jpeg', '.PNG', '.JPG']:
        temp_path = os.path.join(self.mask_dir, mask_name + ext)
        if os.path.exists(temp_path):
            mask_path = temp_path
            break

    if mask_path is None:
        raise FileNotFoundError(f"❌ 找不到掩膜文件: {mask_name}")

    # 3. 读取和预处理图像
    image = cv2.imread(img_path)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # BGR → RGB
    image = cv2.resize(image, self.img_size)        # 调整尺寸

    # 4. 读取和预处理掩膜
    mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
    mask = cv2.resize(mask, self.img_size)
    mask = (mask > 127).astype(np.float32)  # 二值化：>127设为1，否则为0

    # 5. 应用数据增强（如果有）
    if self.transform:
        image = Image.fromarray(image)
        image = self.transform(image)  # 归一化等操作
    else:
        # 转换为 tensor
        image = torch.from_numpy(image).permute(2, 0, 1).float() / 255.0

    # 6. 掩膜转 tensor
    mask = torch.from_numpy(mask).unsqueeze(0).float()

    return image, mask  # 返回图片和标注
```

**数据流程**：
```
原始图片 (1920×1080×3)
  → 调整尺寸 (224×224×3)
  → 归一化 ([0,1])
  → 转换为tensor (3×224×224)

原始标注 (1920×1080)
  → 调整尺寸 (224×224)
  → 二值化 (0或1)
  → 转换为tensor (1×224×224)
```

---

## model_train.py - 模型训练

### 文件作用

训练 U-Net 模型的主程序，包含数据加载、训练循环、验证等。

### 1. 数据加载函数

```python
def train_val_data_process():
    """加载训练和验证数据"""

    # 数据路径
    train_image_path = 'data/train/images'
    train_mask_path = 'data/train/masks'
    val_image_path = 'data/val/images'
    val_mask_path = 'data/val/masks'

    # 归一化参数（预先计算的数据集均值和标准差）
    normalize = transforms.Normalize(
        mean=[0.15804266, 0.16457426, 0.16825973],  # R, G, B 均值
        std=[0.04205786, 0.04393576, 0.04547899]    # R, G, B 标准差
    )

    # 数据预处理流程
    train_transform = transforms.Compose([
        transforms.Resize((224, 224)),  # 调整尺寸
        transforms.ToTensor(),          # 转换为tensor
        normalize                       # 标准化
    ])

    # 创建数据集
    train_data = RoadCrackDataset(train_image_path, train_mask_path,
                                  transform=train_transform)
    val_data = RoadCrackDataset(val_image_path, val_mask_path,
                                transform=train_transform)

    # 创建数据加载器
    batch_size = 91
    train_loader = DataLoader(train_data, batch_size=batch_size,
                             shuffle=True, num_workers=12, pin_memory=True)
    val_loader = DataLoader(val_data, batch_size=batch_size,
                           shuffle=False, num_workers=12, pin_memory=True)

    return train_loader, val_loader
```

**参数说明**：
- `batch_size`: 每次训练使用的图片数量
- `shuffle=True`: 打乱训练数据（验证集不打乱）
- `num_workers`: 使用多少个CPU核心加载数据
- `pin_memory`: 加速GPU数据传输

### 2. IoU 计算

```python
def calculate_iou(pred, target, num_classes=2):
    """
    计算 IoU (Intersection over Union)

    Args:
        pred: 预测结果 [B, H, W]
        target: 真实标注 [B, H, W]
        num_classes: 类别数（背景+裂缝=2）

    Returns:
        平均 IoU
    """
    ious = []
    pred = pred.view(-1)      # 展平为一维
    target = target.view(-1)

    for cls in range(num_classes):
        # 找到预测为该类的像素
        pred_inds = pred == cls
        # 找到真实为该类的像素
        target_inds = target == cls

        # 计算交集和并集
        intersection = (pred_inds & target_inds).sum().float()
        union = (pred_inds | target_inds).sum().float()

        if union == 0:
            ious.append(float('nan'))  # 该类别不存在
        else:
            ious.append((intersection / union).item())

    return np.nanmean(ious)  # 返回平均值
```

**IoU 可视化**：
```
真实标注:     预测结果:     交集:         并集:
█████         ████          ████          █████
█████         ████          ████          █████
█████         █████         █████         █████

IoU = 交集面积 / 并集面积 = 13 / 15 = 0.867
```

### 3. 训练主函数

```python
def train_model_process(model, train_dataloader, val_dataloader, num_epochs):
    """训练模型"""

    # 1. 设备配置
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model = model.to(device)

    # 如果有多个GPU，使用并行训练
    if torch.cuda.device_count() > 1:
        model = nn.DataParallel(model)

    # 2. 优化器和损失函数
    optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
    criterion = nn.CrossEntropyLoss()  # 交叉熵损失

    # 3. 记录最佳模型
    best_model_wts = copy.deepcopy(model.state_dict())
    best_iou = 0.0

    # 4. 记录训练过程
    train_loss_all = []
    val_loss_all = []
    train_iou_all = []
    val_iou_all = []

    # 5. 训练循环
    for epoch in range(num_epochs):
        print(f"Epoch {epoch}/{num_epochs-1}")

        # ===== 训练阶段 =====
        model.train()  # 设置为训练模式
        train_loss = 0.0
        train_iou = 0.0
        train_num = 0

        for images, masks in train_dataloader:
            images = images.to(device)
            masks = masks.to(device, dtype=torch.long)

            # 前向传播
            output = model(images)           # [B, 2, H, W]
            loss = criterion(output, masks)  # 计算损失

            # 反向传播
            optimizer.zero_grad()  # 清空梯度
            loss.backward()        # 计算梯度
            optimizer.step()       # 更新参数

            # 计算预测结果
            pred_mask = torch.argmax(output, dim=1)  # [B, H, W]

            # 累加指标
            train_loss += loss.item() * images.size(0)
            train_iou += calculate_iou(pred_mask, masks) * images.size(0)
            train_num += images.size(0)

        # ===== 验证阶段 =====
        model.eval()  # 设置为评估模式
        val_loss = 0.0
        val_iou = 0.0
        val_num = 0

        with torch.no_grad():  # 不计算梯度
            for images, masks in val_dataloader:
                images = images.to(device)
                masks = masks.to(device, dtype=torch.long)

                output = model(images)
                loss = criterion(output, masks)
                pred_mask = torch.argmax(output, dim=1)

                val_loss += loss.item() * images.size(0)
                val_iou += calculate_iou(pred_mask, masks) * images.size(0)
                val_num += images.size(0)

        # 6. 计算平均指标
        train_loss_all.append(train_loss / train_num)
        train_iou_all.append(train_iou / train_num)
        val_loss_all.append(val_loss / val_num)
        val_iou_all.append(val_iou / val_num)

        print(f'Train Loss: {train_loss_all[-1]:.4f} Train IoU: {train_iou_all[-1]:.4f}')
        print(f'Val Loss: {val_loss_all[-1]:.4f} Val IoU: {val_iou_all[-1]:.4f}')

        # 7. 保存最佳模型
        if val_iou_all[-1] > best_iou:
            best_iou = val_iou_all[-1]
            best_model_wts = copy.deepcopy(model.state_dict())

    # 8. 加载最佳模型并保存
    model.load_state_dict(best_model_wts)
    torch.save(model.state_dict(), 'best_model.pth')

    return train_process  # 返回训练历史
```

**训练流程图**：
```
for each epoch:
  ┌─────────────────────┐
  │   训练阶段          │
  │  1. 前向传播        │
  │  2. 计算损失        │
  │  3. 反向传播        │
  │  4. 更新参数        │
  └─────────────────────┘
          ↓
  ┌─────────────────────┐
  │   验证阶段          │
  │  1. 前向传播        │
  │  2. 计算指标        │
  │  3. 不更新参数      │
  └─────────────────────┘
          ↓
  ┌─────────────────────┐
  │  记录和保存         │
  │  1. 记录Loss/IoU    │
  │  2. 保存最佳模型    │
  └─────────────────────┘
```

---

## app.py - Web 应用

### 文件作用

使用 Gradio 创建 Web 界面，让用户可以通过浏览器使用模型。

### 核心类：CrackDetectionApp

```python
class CrackDetectionApp:
    def __init__(self, model_path='best_model.pth'):
        """初始化应用"""

        # 1. 设备配置
        self.device = "cuda" if torch.cuda.is_available() else "cpu"

        # 2. 加载模型
        self.model = UNet(n_channels=3, n_classes=2)
        self.model.load_state_dict(torch.load(model_path, map_location=self.device))
        self.model = self.model.to(self.device)
        self.model.eval()  # 评估模式

        # 3. 数据预处理
        self.normalize = transforms.Normalize(
            mean=[0.15804266, 0.16457426, 0.16825973],
            std=[0.04205786, 0.04393576, 0.04547899]
        )

        self.transform = transforms.Compose([
            transforms.Resize((224, 224)),
            transforms.ToTensor(),
            self.normalize
        ])
```

### 图片处理流程

```python
def process_image(self, input_image, show_overlay=True, overlay_alpha=0.5):
    """处理上传的图像"""

    # 1. 预处理图片
    image_tensor, original_size = self.preprocess_image(input_image)

    # 2. 模型预测
    pred_mask = self.predict(image_tensor)

    # 3. 计算裂缝占比
    crack_percentage = self.calculate_crack_percentage(pred_mask)

    # 4. 生成可视化
    mask_vis = (pred_mask * 255).astype(np.uint8)  # 0-1 → 0-255
    overlay_image = self.create_overlay(input_image, pred_mask, overlay_alpha)

    # 5. 生成报告
    if crack_percentage > 0.1:
        severity = "检测到裂缝"
        if crack_percentage > 10:
            severity += " - 严重"
        elif crack_percentage > 5:
            severity += " - 中等"
        else:
            severity += " - 轻微"
    else:
        severity = "未检测到明显裂缝"

    report = f"""
    ### 检测结果
    - **裂缝占比**: {crack_percentage:.2f}%
    - **严重程度**: {severity}
    """

    return mask_vis, overlay_image, report
```

### Gradio 界面

```python
def create_gradio_interface():
    """创建 Gradio 界面"""

    app = CrackDetectionApp()

    with gr.Blocks(title="道路裂缝检测系统") as demo:
        gr.Markdown("# 🛣️ 道路裂缝检测系统")

        with gr.Row():
            # 左侧：输入
            with gr.Column():
                input_image = gr.Image(type="pil", label="上传道路图片")
                show_overlay = gr.Checkbox(value=True, label="显示叠加图")
                overlay_alpha = gr.Slider(0, 1, value=0.5, label="叠加透明度")
                predict_btn = gr.Button("🔍 开始检测")

            # 右侧：输出
            with gr.Column():
                output_mask = gr.Image(label="检测结果")
                output_overlay = gr.Image(label="叠加可视化")
                output_report = gr.Markdown(label="检测报告")

        # 绑定事件
        predict_btn.click(
            fn=app.process_image,
            inputs=[input_image, show_overlay, overlay_alpha],
            outputs=[output_mask, output_overlay, output_report]
        )

    return demo
```

---

## 如何修改和扩展

### 1. 调整输入图片尺寸

**修改位置**：`model.py`, `dataset.py`, `app.py`

```python
# 从 224×224 改为 448×448
img_size = (448, 448)

# 注意：需要重新训练模型！
```

**影响**：
- ✅ 更高的检测精度
- ❌ 更慢的推理速度
- ❌ 更大的显存占用

### 2. 增加数据增强

**修改位置**：`model_train.py`

```python
from torchvision import transforms

train_transform = transforms.Compose([
    transforms.Resize((224, 224)),
    # 新增数据增强
    transforms.RandomHorizontalFlip(p=0.5),     # 随机水平翻转
    transforms.RandomVerticalFlip(p=0.5),       # 随机垂直翻转
    transforms.RandomRotation(15),              # 随机旋转±15度
    transforms.ColorJitter(                     # 颜色抖动
        brightness=0.2,
        contrast=0.2,
        saturation=0.2
    ),
    transforms.ToTensor(),
    normalize
])
```

### 3. 修改模型结构

**示例：增加模型深度**

```python
# 在 model.py 中添加一层
self.down5 = nn.Sequential(
    nn.MaxPool2d(2),
    DoubleConv(1024, 2048)
)

# 相应地添加上采样层
self.up0 = nn.ConvTranspose2d(2048, 1024, kernel_size=2, stride=2)
self.conv0 = DoubleConv(2048, 1024)
```

### 4. 添加新的评估指标

**修改位置**：`model_train.py`

```python
def calculate_precision_recall(pred, target):
    """计算精确率和召回率"""
    pred = pred.view(-1)
    target = target.view(-1)

    # 只关注裂缝类别
    tp = ((pred == 1) & (target == 1)).sum().float()  # 真阳性
    fp = ((pred == 1) & (target == 0)).sum().float()  # 假阳性
    fn = ((pred == 0) & (target == 1)).sum().float()  # 假阴性

    precision = tp / (tp + fp + 1e-8)  # 精确率
    recall = tp / (tp + fn + 1e-8)     # 召回率

    return precision.item(), recall.item()
```

### 5. 保存检测结果到文件

**修改位置**：`app.py`

```python
def process_image(self, input_image, ...):
    # ... 原有代码 ...

    # 新增：保存结果
    import datetime
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")

    # 保存mask
    Image.fromarray(mask_vis).save(f"results/mask_{timestamp}.png")

    # 保存叠加图
    Image.fromarray(overlay_image).save(f"results/overlay_{timestamp}.png")

    # 保存报告
    with open(f"results/report_{timestamp}.txt", "w") as f:
        f.write(report)

    return mask_vis, overlay_image, report
```

### 6. 添加多类别分割

**示例：检测多种道路缺陷**

```python
# 修改模型输出类别数
model = UNet(n_channels=3, n_classes=4)  # 背景、裂缝、坑洼、破损

# 修改颜色映射
color_map = {
    0: [0, 0, 0],       # 背景 - 黑色
    1: [255, 0, 0],     # 裂缝 - 红色
    2: [0, 255, 0],     # 坑洼 - 绿色
    3: [0, 0, 255],     # 破损 - 蓝色
}
```



### 实践建议

1. **从小实验开始**：先用少量数据（50张）测试代码
2. **逐步增加复杂度**：先跑通基础版本，再添加功能
3. **多做可视化**：可视化中间结果有助于理解
4. **记录实验**：记录每次修改的参数和结果

---

**恭喜完成代码学习！** 现在你应该对整个项目有了全面的理解。开始动手实践吧！
